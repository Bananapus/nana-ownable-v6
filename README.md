...
- **Future project IDs are only safe when you control mint sequencing.** The constructor intentionally allows ownership to be tied to a project ID that does not exist yet. That is only safe when the deployer also controls how that future project ID is reserved or minted. Otherwise, whoever mints that project NFT first becomes owner.
