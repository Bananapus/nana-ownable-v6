...
Scope: the two production contracts in `src/` — `JBOwnable` (concrete) and `JBOwnableOverrides` (abstract base) — plus their `IJBOwnable` interface in `src/interfaces/` and `JBOwner` struct in `src/structs/`. The package is a Juicebox-aware drop-in replacement for OpenZeppelin's `Ownable`: a contract inheriting `JBOwnable` resolves its owner either to a fixed address (address mode) or dynamically to the current holder of a Juicebox project's ERC-721 NFT (project mode). Project mode can opt into a `JBPermissions` ID that delegates `onlyOwner` access to addresses authorized by the current resolved project owner. Address mode is direct-owner-only.
...
- **A.1.1 Single-source-of-truth resolution.** Every authorization check goes through the same internal helper, `_checkOwner`, which reads `jbOwner` once into memory, resolves the owner address (project NFT holder for `projectId != 0`, stored address for `projectId == 0`), and either bypasses `JBPermissions` (always in address mode, or in project mode when `permissionId == 0` or stale — see A.3.2) or routes through `_requirePermissionFrom`. There is no second auth path on the contract.
...
- **A.1.4 Address mode is fixed until transfer.** When `jbOwner.projectId == 0`, the owner is `jbOwner.owner` exactly — no project lookup, no fallback, and no `JBPermissions` lookup. The address only changes via `transferOwnership`, `transferOwnershipToProject`, or `renounceOwnership`.

## A.2 Caller authorization

- **A.2.1 Direct owner always passes.** The resolved owner can always call `onlyOwner` functions, regardless of the stored `permissionId` value. Address mode uses a plain `_msgSender() == resolvedOwner` comparison and bypasses `JBPermissions` entirely.
- **A.2.2 Delegate-by-permission is project-mode opt-in.** Delegation is OFF by default (`jbOwner.permissionId` is zero after construction unless the inheriting constructor sets it). The current project owner must explicitly call `setPermissionId(nonzero)` for any `JBPermissions` grant to grant `onlyOwner` access on this contract. In address mode, `setPermissionId(nonzero)` reverts and `_checkOwner` ignores `JBPermissions`.
...
- **A.2.5 No ROOT-on-disabled or wildcard-address bypass.** `_checkOwner` calls `_requirePermissionFrom({account: resolvedOwner, projectId: ownerInfo.projectId, permissionId: effectivePermissionId})` only in project mode when `effectivePermissionId != 0`. Address mode and disabled/stale project-mode delegation take the explicit-equality branch and never enter the permissions system. A third party who holds `ROOT` or a wildcard project permission through `JBPermissions` cannot call `onlyOwner` unless the contract is project-owned and the owner has set a non-zero `permissionId`.
...
- **B.1.4 Set or clear the delegation permission ID.** The owner can call `setPermissionId(uint8 permissionId)`. Setting to 0 disables delegation entirely. Setting a non-zero value is only allowed in project mode and also snapshots the current resolved project owner into `_permissionOwner`.
...
- **B.2.5 Cannot bypass authentication via direct storage write.** `jbOwner` is `public` (auto-getter) but write access is internal-only. The only mutating paths are `_transferOwnership` and `_setPermissionId`; public calls to them are `_checkOwner`-gated, and nonzero permission IDs are rejected in address mode. An inheriting contract that exposes a raw setter is its own bug, not this base's.
...
- **`setPermissionId(uint8 permissionId) public virtual override`** — owner only. Calls `_checkOwner`; if the contract is address-owned, nonzero IDs revert. Otherwise calls `_setPermissionId(permissionId)` (which writes `jbOwner.permissionId = permissionId`, sets `_permissionOwner = owner()` after the write, and emits `PermissionIdChanged`).
...
- **`_checkOwner() internal view virtual`** — the auth primitive. Loads `jbOwner`; address mode directly compares `_msgSender()` to the stored owner and returns without consulting `JBPermissions`. Project mode resolves the current owner with fail-closed fallback to `address(0)`, zeroes a stale `permissionId` if `resolvedOwner != _permissionOwner`, then either does `_msgSender() == resolvedOwner` (delegation disabled/stale, no permissions check) or calls `_requirePermissionFrom({account: resolvedOwner, projectId: ownerInfo.projectId, permissionId: effectivePermissionId})`.
...
- **`_setPermissionId(uint8 permissionId) internal virtual`** — writes `jbOwner.permissionId = permissionId`, then sets `_permissionOwner = owner()`. **Order matters:** `_permissionOwner` is set AFTER the `permissionId` write, but is read AFTER the resolved owner is computed via `owner()`; the two operations cannot interleave because Solidity has no concurrency. If `setPermissionId(X)` is called by a project-mode delegate (not the direct owner), `_permissionOwner` is still set to the current resolved owner, not the caller — delegates can rotate the permission ID; they cannot rebase delegation onto themselves.
...
- **`JBOwner public jbOwner`** (`src/JBOwnableOverrides.sol:46`) — packed `(address owner, uint88 projectId, uint8 permissionId)` totalling 184 bits, fits in one storage slot. `owner` only used when `projectId == 0`. `permissionId == 0` means "no delegation"; nonzero values are only valid for project-owned contracts.
...
- **`IJBPermissions public immutable PERMISSIONS`** — inherited from `JBPermissioned`, used by `_requirePermissionFrom` indirectly during project-owned delegation checks.
...
- **D.6 ROOT or wildcard permissions are not bypasses.** Address mode never queries `JBPermissions`, so wildcard project grants cannot control an address-owned contract. In project mode, when delegation is disabled (`permissionId == 0` or stale), `_checkOwner` takes the direct-equality branch and never queries `JBPermissions`. A holder of `ROOT` on the resolved owner cannot call `onlyOwner` through delegation that isn't enabled — they would have to hold the project NFT directly (A.2.5). This closes the "owner sells NFT, ROOT delegate keeps control" attack class.
- **D.7 `setPermissionId` snapshots the project owner at write time, not the caller.** A delegate who is currently authorized (`effectivePermissionId != 0` AND a valid `JBPermissions` grant) can call `setPermissionId` while the contract is project-owned — it passes `_checkOwner`. But `_permissionOwner = owner()` is set to the CURRENT RESOLVED OWNER, not `_msgSender()` (`src/JBOwnableOverrides.sol:239`). Delegates can rotate the ID; they cannot rebase delegation onto themselves. Address-owned contracts reject nonzero permission IDs.
...
- **`JBPermissions`.** Project-owned delegation routes through `PERMISSIONS.hasPermission`. The permissions contract cannot be swapped after `JBOwnable` deployment (`PERMISSIONS` is immutable). The permissions contract itself has no global admin — see `../INVARIANTS.md` Section C.8.
...
| A.2.2, B.1.4 (project-owned delegation opt-in) | `src/JBOwnableOverrides.sol:181–184, 237–241` |
...
| A.2.5, D.6 (no ROOT or wildcard bypass when delegation off) | `src/JBOwnableOverrides.sol:151–158, 160–162` |
